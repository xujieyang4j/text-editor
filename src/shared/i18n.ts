...
const COMMAND_ZH: Partial<Record<MenuEvent, string>> = {
  ...
  'to-upper-case': '转为大写', 'to-lower-case': '转为小写', 'to-title-case': '转为标题格式', 'swap-case': '反转大小写', 'join-lines': '合并行', 'trim-trailing-whitespace': '删除行尾空白', 'indent-selection': '增加缩进', 'outdent-selection': '减少缩进', 'reindent-selection': '自动重新缩进选区', 'convert-eol-lf': '将换行符转换为 LF', 'convert-eol-crlf': '将换行符转换为 CRLF', 'convert-eol-cr': '将换行符转换为 CR',
