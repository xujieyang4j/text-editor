...
- **文件树侧栏**：默认收起（`Ctrl/Cmd+B` 展开），可把文件夹作为工作区打开并按需加载目录；执行
  *View: Reveal Active File in Sidebar* 会自动显示侧栏、退出专注模式、展开父目录并高亮当前已保存文件，
  但不会修改文件内容
...
- **完整编码工作流**：状态栏可分别选择“保存编码”或“以编码重新打开”。支持 `UTF-8`、
  `UTF-8 BOM`、带/不带 BOM 的 `UTF-16 LE/BE`、`GB18030`、`GBK`、`Big5`、`Shift-JIS`、
  `Windows-1252` 与 `ISO-8859-1`，以及 `LF`、`CRLF`、`CR` 换行符。BOM 和严格 UTF-8
  会自动识别，传统编码由用户明确选择，避免有歧义的自动猜测。也可通过“以编码打开文件…”直接打开
  无 BOM UTF-16 或传统编码文件。传统编码无法无损表示文档字符时，保存会被拒绝而不会静默写成问号
 ；包含无效字节或仅靠启发式推测编码的文档，在确认编码前也不能覆盖源文件（包括通过“另存为”别名覆盖）
- **原生应用菜单**，各平台均带标准键盘快捷键
...
普通用户请直接从 [GitHub Releases](https://github.com/xujieyang4j/text-editor/releases) 下载：
Windows x64 提供安装版和便携版，macOS 分 Apple Silicon (`arm64`) 与 Intel (`x64`)，Linux x64
提供 AppImage 与 deb。正式 Release 的 Windows 文件已签名，macOS 文件已签名并完成 Apple 公证，
同时附带 `SHA256SUMS.txt`。
...
这些 `dist:*` 命令用于本地构建验证，不等同于正式 GitHub Release。正式发布只通过仓库的 tag
工作流完成，工作流会强制验证 Windows 签名以及 macOS Developer ID 签名和 Apple 公证。
