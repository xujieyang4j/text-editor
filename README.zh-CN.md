...
- **行操作**：上/下移动行、复制行、删除行、按词删除、删除至行首/行尾、复制选区、在上方/下方新建空行（`Ctrl/Cmd+Shift+Enter` / `Enter`）、转置相邻字符（`Ctrl+T`）、合并行、切换注释（`Ctrl/Cmd+/`）、行排序
- **文件树侧栏**：默认收起（`Ctrl/Cmd+B` 展开），可把文件夹作为工作区打开并按需加载目录
- **查找替换**、**文本撤销/重做**、**多光标**、矩形选择、整行/匹配括号/外层语法结构选择、按行拆分选区、逐级扩展/缩小选区、多选区大小写转换、括号匹配
