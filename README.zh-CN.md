...
- **语法高亮**：支持 100+ 语言，按文件扩展名自动识别（基于 `@codemirror/language-data`）
- **文件树侧栏**：把文件夹作为工作区打开，按需懒加载展开子目录
- **查找与替换**（`Ctrl/Cmd+F`、`Ctrl/Cmd+H`）与**跳转到行**（`Ctrl/Cmd+G`）
- **撤销/重做**、**多光标**、矩形选择、括号匹配、自动补全括号
- **自动补全**、代码折叠、当前行高亮、选中词高亮
- **状态栏**：行/列、选中字符数、语言、编码、换行符
- **明暗主题**切换、软**换行**切换、可折叠侧边栏
- **原生应用菜单**，各平台均带标准键盘快捷键
